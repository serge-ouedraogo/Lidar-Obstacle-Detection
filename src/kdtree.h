...
struct KdTree
{
  ...
  void inserthelper(Node **root, const std::vector<float>& point, int depth, int id)
  {
    if(*(root) == NULL)
    {
  ...
  void searchhelper(const std::vector<float>& target, Node *node, int depth, float distTol, std::vector<int>& ids)
  {
    if(node !=NULL)
    {
